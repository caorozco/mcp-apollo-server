...
#
# USAGE:
# 1. Build: docker build -f mcp.Dockerfile -t nextgenoss-mcp .
# 2. Run:   docker run -p 8000:8000 --env-file .env nextgenoss-mcp
...
LABEL org.opencontainers.image.title="nextgenoss-mcp MCP Server"
LABEL org.opencontainers.image.description="Apollo MCP Server for nextgenoss-mcp GraphQL API"
